...
def fetch_active_sprint_tasks() -> dict:
    ...
    for item in raw_items:
        ...
        # Enforce scope: Task work items only (section 5.1)
        if work_item_type not in Config.ALLOWED_WORK_ITEM_TYPES:
            logger.debug(
                "Skipping work item %d of type '%s'", item.get("id"), work_item_type
            )
            continue
        ...
        logger.debug("Fetching comments for task %d...", task_id)
        comments = get_comments_for_work_item(task_id)

        today_comments = [c for c in comments if c["is_today"]]
        has_comment_today = len(today_comments) > 0
        today_comment_text = today_comments[0]["text"] if today_comments else None
        recent_comment_texts = [
            c["text"] for c in comments if not c["is_today"]
        ]
