...
# Gap: "LLM Reliability" (Med) — explicit schema validation with one
# retry-with-correction attempt, same pattern as comment_analyst.py.
_NUDGE_SCHEMA = {
    "type": "object",
    "required": ["message", "tone"],
    "properties": {
        "message": {"type": "string", "minLength": 1},
        "tone": {"type": "string", "enum": ["gentle", "direct", "firm"]},
    },
}
...
@retryable_any_exception("groq-nudge-write")
def _call_groq_nudge(messages: list[dict]) -> str:
...
def _write_with_correction(messages: list[dict]) -> dict | None:
    for attempt in range(2):
        try:
            raw = _call_groq_nudge(messages)
        except Exception as exc:
            logger.warning("Groq nudge-write call failed after retries: %s", exc)
            return None

        try:
            parsed = json.loads(_strip_code_fences(raw))
            jsonschema_validate(parsed, _NUDGE_SCHEMA)
            return parsed
        except (json.JSONDecodeError, ValidationError) as exc:
            if attempt == 0:
                logger.info(
                    "Groq nudge response failed schema validation (%s) — "
                    "retrying with correction prompt.", exc,
                )
                ...
                continue
            logger.warning(
                "Groq nudge response still invalid after correction attempt "
                "(%s) — using template fallback.", exc,
            )
            return None
    return None
...
def write_nudge_message(
    ...
    result = _write_with_correction(messages)
